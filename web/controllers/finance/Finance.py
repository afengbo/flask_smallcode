...
from common.libs.Helper import iPagination, select_filter_from_obj, getDictListFilterField, get_dict_by_field, get_current_time
...
def index():
    ...
    pay_list = query.order_by(
        PayOrder.id.desc()).offset(offset).limit(
...
def set():
    ...
    stat_info = db.session.query(
        PayOrder, func.sum(
            PayOrder.total_price).label("total")) .filter(
