...
from common.libs.Helper import ops_render, get_current_time, get_dict_by_field, iPagination
...
def info():
    ...
    stock_change_list = FoodStockChangeLog.query.filter(
        FoodStockChangeLog.food_id == id) .order_by(
...
def cat():
    ...
    list = query.order_by(FoodCat.weight.desc(), FoodCat.id).all()
    resp_data['list'] = list
    resp_data['status_mapping'] = app.config['STATUS_MAPPING']
    resp_data['search_con'] = req
