...
from common.libs.Helper import getFormatDate, iPagination, get_dict_by_field, select_filter_from_obj
...
def index():
    ...
    default_date_from = getFormatDate(
        date=date_before_30days, format="%Y-%m-%d")
    ...
    list = query.order_by(
        StatDailySite.id.desc()).offset(offset).limit(
...
def food():
    ...
    list = query.order_by(
        StatDailyFood.id.desc()).offset(offset).limit(
        app.config['PAGE_SIZE']).all()
    date_list = []
    if list:
        food_map = get_dict_by_field(
            Food, Food.id, "id", select_filter_from_obj(
                list, "food_id"))
        for item in list:
...
def memebr():
    ...
    list = query.order_by(
        StatDailyMember.id.desc()).offset(offset).limit(
        app.config['PAGE_SIZE']).all()
    date_list = []
    if list:
        member_map = get_dict_by_field(
            Member, Member.id, "id", select_filter_from_obj(
                list, "member_id"))
        for item in list:
