...
from common.libs.Helper import getFormatDate, get_current_time
...
eg: python manage.py runjob -m stat/daily -a member -p 2020-03-06
...
class JobTask():
    ...
    def run(self, params):
        ...
        date = params['param'][0] if params['param'] and len(
            params['param']) else getFormatDate(format="%Y-%m-%d")
        if not act:
            return
