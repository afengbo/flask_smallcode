...
class PayService():
    ...
    def createOrder(self, member_id, items=None, params=None):
        ...
        try:
            # 为了防止并发库存出问题，这里使用数据库锁，我们可以做select update操作验证下
            ...
            model_pay_order.updated_time = model_pay_order.created_time = get_current_time()
            ...
            for item in items:
                ...
                tmp_pay_item.updated_time = tmp_pay_item.created_time = get_current_time()
    ...
    def orderSuccess(self, pay_order_id=0, params=None):
        try:
            ...
            for order_item in pay_order_items:
                ...
                tmp_model_sale_log.created_time = get_current_time()
    ...
    def addPayCallbackData(self, pay_order_id=0, type='pay', data=''):
        ...
        model_callback.created_time = model_callback.updated_time = get_current_time()
